var settings = SettingStorageData;

chrome.runtime.onMessage.addListener(messageListener);

SettingStorage.load(function (result) {
    settings = result;
});
...
function messageListener(message, sender) {
    ...
    var args    = [],
        isGroup = ["group", "groupEnd"].indexOf(message.log_type) > -1;
    if (!isGroup && settings.showFileAndLineInformation && message.meta.File.length > 0) {
